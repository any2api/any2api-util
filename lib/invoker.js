...
var readInput = function(input, callback) {
  ...
  input.specPath = input.specPath || input.path || process.env.APISPEC_PATH;
  ...
  async.series([
    ...
    function(callback) {
      var getAccess = function(data, callback) {
        ...
        ctx.invokerConfig = ctx.invokerConfig || data.parameter.value || {};
      ...
      var writeParamFilesStream = streamUtil.writeParameterFilesStream({
      ...
      _.each(ctx.pipeline, function(stream) {
        if (_.isString(stream) && stream === 'writeParameterFilesStream') {
          lastStream = lastStream.pipe(writeParamFilesStream);
        } else if (_.isString(stream) && stream === 'readResultFilesStream') {
  ...
  return function(spec) {
    ...
    obj.invoke = function(args, done) {
      ...
      prepareInvoke(args, function(err, ctx) {
        ...
        ctx.pipeline.push('writeParameterFilesStream');
        ...
        ctx.pipeline.push('readResultFilesStream');
