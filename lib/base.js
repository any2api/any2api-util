...
var updateInvokers = function(args, done) {
  ...
  _.each(apiSpec.invokers, function(props, name) {
    if (props.path && !fs.existsSync(path.join(path.dirname(apiSpec.path), props.path, 'invoker.json'))) {
...
var prepareInvoker = function(args, done) {
  ...
  if (!invokerName && apiSpec.executables[args.executableName]) {
    invokerName = apiSpec.executables[args.executableName].invoker_name;
  }
  ...
  var options = { 
    cwd: path.resolve(apiSpec.path, '..', invoker.path),
    env: process.env || {},
...
var prepareExecutable = function(args, done) {
  ...
  debug('preparing executable', apiSpec.executables[args.executableName]);

  var options = {
    cwd: path.resolve(apiSpec.path, '..', invoker.path),
    env: {
      APISPEC_INVOKER: JSON.stringify(invoker),
      APISPEC_INVOKER_NAME: executable.invoker_name,
      APISPEC_EXECUTABLE: JSON.stringify(executable),
      APISPEC_EXECUTABLE_NAME: args.executableName,
      APISPEC_PATH: apiSpec.path
    },
    timeout: args.timeout || childProcTimeout,
    killSignal: childProcKillSignal,
    maxBuffer: childProcMaxBuffer
  ...
  childProc.exec('npm run prepare-executable', options, function(err, stdout, stderr) {
    ...
    specUtil.readSpec({ specPath: apiSpec.path }, function(err, apiSpec) {
      if (err) return done(err);

      apiSpec.executables[args.executableName].prepared = true;
...
var persistExecutable = function(args, done) {
  ...
  temp.mkdir('tmp-executable-' + args.executableName, function(err, execPath) {
...
var runInstance = function(args, done) {
  ...
  var invokerName = args.invokerName;
  var executableName = args.executableName;
  ...
  async.series([
    ...
    function(callback) {
      if (executableName) {
        executable = apiSpecCopy.executables[executableName];

        invoker = apiSpecCopy.invokers[executable.invoker_name];
      } else if (invokerName) {
        invoker = apiSpecCopy.invokers[invokerName];

        if (instance.executable) {
          executable = instance.executable;

          executableName = executableName || 'embedded-' + shortId.generate();

          executable.invoker_name = invokerName;

          apiSpecCopy.executables[executableName] = executable;
        }
      }

      if (_.isEmpty(invoker)) return callback(new Error('invoker missing'));
    ...
    function(callback) {
      if (_.isEmpty(executable) || !executable.files) return callback();

      persistExecutable({ executable: executable, executableName: executableName }, callback);
    },
    function(callback) {
      if (_.isEmpty(executable) || executable.prepared) return callback();
    ...
    function(callback) {
      ...
      var options = {
        ...
        env: {
          APISPEC_INVOKER: JSON.stringify(invoker),
          APISPEC_INVOKER_NAME: invokerName,
          APISPEC_PATH: apiSpecCopy.path,
      ...
      if (!_.isEmpty(executable)) {
        options.env.APISPEC_EXECUTABLE = JSON.stringify(executable);
        options.env.APISPEC_EXECUTABLE_NAME = executableName;
      }
    ...
    async.parallel([
      function(callback) {
        fs.remove(apiSpecCopy.path, callback);
      }
    ], function(err2) {
