...
var readSpec = function(args, callback) {
  ...
  args.specPath = args.specPath || args.path;
...
var writeSpec = function(args, callback) {
  ...
  var specPath = args.specPath || apiSpec.path;
  ...
  writeFile(function(err) {
    ...
    if (specPath) apiSpec.path = specPath;
...
var cloneSpec = function(args, callback) {
  ...
  clonedSpec.path = path.join(path.dirname(apiSpec.path), 'tmp-apispec-' + shortId.generate() + '.json');
...
var enrichSpec = function(args, done) {
  ...
  var basePath = args.basePath || path.dirname(apiSpec.path);
